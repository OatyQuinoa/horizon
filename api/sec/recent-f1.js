/**
 * Vercel serverless: proxy SEC browse-edgar Atom feed for F-1/F-1A (foreign issuer registration).
...
function parseSecAtomFeed(xml) {
  ...
  while ((match = entryRegex.exec(xml)) !== null) {
    ...
    const formType = (categoryMatch && categoryMatch[1]) ? categoryMatch[1] : 'F-1';
    const accessionMatch = id.match(/accession-number=([\w-]+)/i);
    const accessionNumber = accessionMatch ? accessionMatch[1] : id.replace(/^.*,/, '');
    const titleCompanyMatch = title.match(/^(?:F-1\/?A?)\s*[-–]\s*(.+?)\s*\((\d+)\)\s*(?:\(Filer\))?$/i)
      || title.match(/^([^(]+?)\s*\((\d+)\)\s*(?:\(Filer\))?$/i);
    const cik = titleCompanyMatch ? titleCompanyMatch[2].padStart(10, '0') : '';
    const companyName = titleCompanyMatch ? titleCompanyMatch[1].trim() : title.replace(/^(?:F-1\/?A?)\s*[-–]\s*/i, '').replace(/\s*\(\d+\)\s*(?:\(Filer\))?$/, '').trim();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('SEC recent-f1 proxy error:', err);
    res.setHeader('Content-Type', 'application/json');
